...
An Angular **22.1** workspace (project `agents-ui`, prefix `app`) laid out by `.claude/rules/ui-architecture.md`, which auto-applies under `src/app/`. Configured, not yet a product: an app shell, one lazy page (`pages/home/`) and one root store with its toggle. There is still no dev proxy to `agents-api/`, but every visitor now signs in automatically through Microsoft Entra ID before anything renders. `public/config.json` — the shared app registration and the API's base URL, never compiled in — is fetched at runtime; the deployment pipeline overwrites it per environment after the build. `src/main.ts` short-circuits at `/auth`: no config fetch, no Angular bootstrap, only msal-browser's redirect bridge relaying the response back to MSAL, because a dedicated page is what msal-browser 5 requires for that hand-back. `core/auth/auth-store.ts` (`AuthStore`, an NgRx SignalStore) drives sign-in from `provideAppInitializer`, which blocks first render, with a `sessionStorage`-stamped loop breaker so a sign-in that can't complete fails instead of redirecting forever; `core/auth/token-refresh-service.ts` (`TokenRefreshService`) then renews the token in the background — a forced refresh at load, a timer inside MSAL's renewal window, and an immediate check on reconnect or when a backgrounded tab turns visible. `core/auth/msal.interceptor.ts` wraps msal-angular's class-based `MsalInterceptor` in a functional `HttpInterceptorFn`, since Angular 22.1's typings say DI-provided interceptors may be phased out. Tokens live in `localStorage`, so a signed-in tab stays signed in after it's closed and a new tab starts already signed in; there is no sign-out anywhere in the app, by design. See [docs/ui/authentication.md](../docs/ui/authentication.md).

- **Stack.** No `zone.js` (the v22 default, and TestBed is zoneless too). `src/testing/` now holds its first fixtures — `app-settings.ts` and a fake `IPublicClientApplication` in `msal.ts` — but still no providers file that wires them all together for a whole-app test. TypeScript **6.0** — the compiler rejects 6.1 and 7 — with `paths` aliases `@pages/*` … `@testing/*` and no `baseUrl`, which TS 6 deprecates. Vitest **4** through `ng test` (the builder rejects Vitest 5) on jsdom. `@ngrx/signals` 22. npm; Node 22.22+ or 24.15+.
- **Bootstrap is compiled from Sass with the brand palette.** `src/styles.scss` loads it with `@use 'bootstrap/scss/bootstrap' with (...)`: Summit Navy `primary`, Slate `secondary`, Glacier Blue `info`, Andes Green `success`, Ink on Snow in light, Snow on Ink with Summit Navy surfaces in dark. Components, focus rings and button text all derive from those values; only Bootstrap Icons stays precompiled CSS in `angular.json`. Angular compiles Sass with `quietDeps`, so Bootstrap's internal deprecations stay silent, and `@use … with` adds none (a `@import` in our own file would). Components generate without a stylesheet (`"style": "none"`). The initial budget warns at 900 kB (raised from 750 kB) and errors at 1 MB, unchanged: Bootstrap and Icons CSS are ~315 kB raw, and `@azure/msal-browser` — ~280 kB minified, tree-shakes poorly, and must ship in the initial chunk because sign-in blocks first render — pushed the initial total to 887.57 kB raw.
...
- **Verified so far:** `ng build` with no warnings at the 900 kB budget (initial 887.57 kB raw), `ng test` (43 tests), `ng lint`, `prettier --check`, and the initial-chunk gate passing; also, from before sign-in was added, the gate failing on a statically imported page, every layer ban firing — and not firing — on deliberate violations, and headless Edge against `ng serve`: no `zone.js` and no console errors, the palette in both themes (computed colors of buttons, links, muted text, focus rings, checked inputs, pills, list-group, pagination, progress and alerts, every text pair at AA or better), the icon font, the lazy home page, the tooltip (its text updating while open), the OS scheme followed while nothing is stored, and a toggle persisting across a reload. Headless Edge against `ng serve` for sign-in: `config.json` fetched before the Entra `authorize` navigation, which carried the configured tenant and `client_id`, `redirect_uri=<origin>/auth`, `<client>/.default` with `offline_access`, and PKCE S256, while only "Signing in…" was on screen and no `zone.js` loaded; a substituted `config.json` reaching that URL without a rebuild; a 404 and a missing `AzureAd.ClientId` each showing the configuration error with no Entra navigation; and `/auth` without a response never bootstrapping, fetching no config and falling back to `/`. **Not yet verified:** a real Entra ID sign-in through a browser, token claims (`aud`, `ver`, `scp`), whether a second silent `.default` call is served from cache, and a browser-driven call to `agents-api/`.
