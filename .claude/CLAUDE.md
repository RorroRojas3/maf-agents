...
- **Policy database — SQL Server 2025 through EF Core 10.** `PolicyDbContext` maps one entity, `[Core].[Policy]`: a sequential-GUID clustered key, a unique `PolicyNumber`, a holder index covering the list columns, a `(Status, ExpirationDate)` index for renewal and expiry sweeps, check constraints on status names, the coverage window, both amounts and the currency code, `rowversion` concurrency, and `DateCreated`/`DateUpdated` stamped by `AuditTimestampInterceptor`. Database layer only, by owner decision: no repository, service or endpoint uses it yet, but `SqlDb` is required and `/health/ready` includes it.
...
- **A `SqlException` message quotes data** — the duplicate key for 2601/2627, the truncated value for 2628. Two guards keep it out of sinks: `ConfigureSqlServer` lowers EF's `SaveChangesFailed`, `QueryIterationFailed` and `CommandError` events from `Error` to `Debug`, and a future repository translates the exception so `GlobalExceptionHandler` logs only its type. EF sensitive-data logging stays off.
...
- **The EF context is `PolicyDbContext`** (the owner's name), not `<RootShort>DbContext`.

**Design notes** — these follow the rule; recorded because the reason is not obvious.

- **EF Core lives only in `Repository/Sql/`** (`DbContexts/`, `Configurations/Policies/`, `HealthChecks/`, `Interceptors/`, `Migrations/`, `Provisioning/`); `Repository/Cosmos/` stays on the raw SDK and has no `DbContexts/`, `Configurations/`, `Interceptors/` or `Migrations/`.
- **`Api/Startup/CosmosBootstrapper.cs` and `SqlBootstrapper.cs` are in Api, not the provider folders.** `Program.cs` runs `IStartupValidator.Validate()` before provisioning or migrating, so nothing external is touched by a process about to fail on its own configuration; a hosted service in Repository would run after that guard.
...
Nothing sensitive is committed. `appsettings.json` ships every key with secrets blank; local values go into `dotnet user-secrets` (the Api project has a `UserSecretsId`) or environment variables (`Foundry__ApiKey`, `CosmosDb__Key`, `SqlDb__ConnectionString`, …). With `KeyVault:Enabled`, secrets named with `--` (`Foundry--ApiKey`, `SqlDb--ConnectionString`) layer on top through `DefaultAzureCredential`. The Foundry API key has no managed-identity equivalent, which is the one accepted carve-out from the prefer-`DefaultAzureCredential` standard; Cosmos uses the shared credential whenever `CosmosDb:Key` is blank. SQL Server signs in however `SqlDb:ConnectionString` says — SQL authentication for the local container, `Authentication=Active Directory Managed Identity` in Azure.
