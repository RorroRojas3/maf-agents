...
2. **Load the right rules.** The detailed standards in `.claude/rules/` auto-apply by path as you read matching files; if one is not already in context, `Read` the matching rule directly. The rules that exist are `csharp`, `aspnet-rest-apis`, `api-architecture`, `azure-functions-csharp` and `terraform` — and `api-architecture` is the one to read before judging where a file lives, what it is called, or which project owns it.
