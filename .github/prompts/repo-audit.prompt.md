...
- **`skills-mirror/differs`** — decide the canonical side: `git status` / `git diff` (run in the terminal) shows which side carries the uncommitted (newer) change; copy it over the other in the terminal (e.g. `cp .claude/skills/<path> .github/skills/<path>`). If both sides are committed and differ, do not guess — present both versions to the user.
- **`skills-mirror/missing-*`** — copy the file into the tree that lacks it (or confirm with the user that it should be deleted from both).
...
**Do not commit, do not push, do not open a PR.** This repo _is_ the guidance, and a silently wrong "repair" would surface weeks later as drifted standards. `git diff` is the review surface and `git checkout --` is the undo; a human decides.
