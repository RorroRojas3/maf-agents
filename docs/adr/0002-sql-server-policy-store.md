...
- **`SqlDb` is a required dependency, and the table stays database-layer-only for now.** Startup validates `SqlDb:ConnectionString` unconditionally and `/health/ready` includes a `sql` check, even though no request path touches `Policy` yet — an owner decision to prove the store is live and healthy before anything is built on top of it, rather than making the dependency optional until a consumer exists.
